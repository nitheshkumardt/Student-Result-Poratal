...
import com.Bean.CollectionOfFunctions;
...
//		out.print("uname "+uname+"  "+"password "+password);
		...
		try {
			if(check.check(uname, password)) {
				log.info("Successfully Logged In  "+ uname);
				CollectionOfFunctions.getClasses(uname);
				...
				log.info("Failure To Logged In  "+uname);
				response.sendRedirect("Login.jsp");
			}
		} catch (SQLException | IOException e) {
			log.error("Error  "+uname);
